...
const recentActivity = [
  { time: "2 min ago", event: "New video uploaded — “Spring Boot REST API”", actor: "alice_dev", action: "pending" },
  { time: "14 min ago", event: "User banned — spam behavior", actor: "Admin", action: "banned" },
  { time: "1h ago", event: "Video approved — “Kafka tutorial”", actor: "Admin", action: "approved" },
];
          ...
          <div className="mt-6 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950">
            ...
            <div className="divide-y divide-slate-800">
              {recentActivity.map((item) => (
                <div key={item.time} className="grid grid-cols-4 gap-4 px-5 py-4 text-sm text-slate-200 sm:grid-cols-[1.5fr_2fr_1fr_1fr]">
                  <span>{item.time}</span>
                  <span>{item.event}</span>
                  <span>{item.actor}</span>
                  <span className={`rounded-full px-3 py-1 text-xs ${item.action === "approved" ? "bg-emerald-500/15 text-emerald-300" : item.action === "pending" ? "bg-amber-500/15 text-amber-300" : "bg-rose-500/15 text-rose-300"}`}>
                    {item.action}
                  </span>
                </div>
              ))}
