...
users(id UUID PK, email VARCHAR UNIQUE, password_hash VARCHAR,
      username VARCHAR UNIQUE, display_name VARCHAR, bio TEXT,
      profile_picture_url VARCHAR, is_active BOOLEAN,
...
videos(id VARCHAR PK, title VARCHAR, description TEXT, category_id VARCHAR FK,
       ...
       language VARCHAR(10), status ENUM(pending,processing,ready,failed),
       created_at TIMESTAMP, updated_at TIMESTAMP)
...
The gateway validates JWT and forwards `X-User-Id`. Services must only accept traffic from the gateway and/or verify an internal auth signal (X-Internal-Token or mTLS). Do not expose service ports publicly.
